...
export interface MhzalyTrackerCallbacks {
...
export class MhzalyTracker {
  private video: HTMLVideoElement;
  private overlay: HTMLCanvasElement;
  private callbacks: MhzalyTrackerCallbacks;
  ...
  constructor(
    ...
    callbacks: MhzalyTrackerCallbacks
  ) {
    this.video = video;
  ...
  private drawOverlay(landmarks: NormalizedLandmark[][]): void {
    ...
    for (const lm of landmarks) {
      ...
      ctx.strokeStyle = pinched ? "#80e5ff" : "rgba(0,210,255,0.5)";
      ...
      ctx.fillStyle = pinched ? "#80e5ff" : "rgba(0,210,255,0.7)";
